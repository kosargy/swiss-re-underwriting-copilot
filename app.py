...
def render_value_add_workflow() -> None:
    ...
    with current_tab:
        ...
        if st.session_state.get("analysis_depth") == "Full analysis":
    ...
    if st.session_state.get("analysis_depth") == "Quick underwriting":
        ...
        st.download_button(
            ...
            mime="application/pdf",
            use_container_width=True,
            key="download_value_add_memo_quick",
        )
...
def render_development_workflow() -> None:
    ...
    if abs(probability_total - 100.0) > 0.001:
        ...
        if st.session_state.get("analysis_depth") == "Quick underwriting":
            development_memo = build_development_memo(development_project, comparison)
            st.download_button(
                "Download Investment Committee memo (PDF)",
                ...
                mime="application/pdf",
                use_container_width=True,
                key="download_development_memo_quick",
            )
            render_project_workspace(
                ...
                key_prefix="dev_workspace_quick",
            )
            return
